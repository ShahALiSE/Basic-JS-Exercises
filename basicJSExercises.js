...
var hiding = function (str) {
  
   return str.substr(0, 3)+"*".repeat(str.length - 5)+ str.slice(-2);

}

var protect_email = function (email){
     var arr = email.split("@");
     return hiding(arr[0]) + "@" + arr[1].toString();
}
...
function remove_first_occurrence(text, searchstring) {
  var txt = text.replace("searchstring"," ");
...
function guessing_game(guess) {
 ...
 if (guess == number)
   ...
   alert('Not matched'+", it was "+number);
}
